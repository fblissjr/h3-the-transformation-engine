...
- **Shot headers carry no cut time.** Owner ruling, departing from a rule base 4.2 and ref 5.1 both state: the engine now writes `[Shot 2] The camera cuts to ...` where it wrote `[Shot 2] At 00:05.000, the camera cuts to ...`. Recorded in the contract as `shot-header-no-cut-time`, with the evidence it rests on: one same-seed pair, not blind, in which both renders cut twice in nearly the same places and neither was judged worse. So the ruling is safe on that evidence and its claimed benefit is unshown.

  The goldens were not edited. They stay byte-exact against the guides, and `withoutHeaderTimes` in `src/core/ir/examples.ts` removes the vendor's header times and capitalises the letter after them. The serializer test compares through it, and the planner is shown its example through it too, so the model never sees the format it must not write. Neither comparison can see the ruling on its own: a serializer still writing the times, paired with a transform that did nothing, would pass both. So separate assertions read the rendered header directly, and one checks that changing `cutAtMs` leaves the text unchanged. Putting the old renderer back turns seven tests red.

  `shots[].cutAtMs` stays as the plan's pacing: the planner still places cuts, the editor labels the field as not written into the prompt, and the four cut-time diagnostics still check that the plan is coherent. The FL2VA and L2VA alignment line keeps its seconds. It pins a frame, not a cut.

  `CUT_TIMESTAMP_IN_PROSE` is gone, because under the ruling it fires on legitimate output: a time that splits action inside a shot is the one use of a time the ruling keeps. Three codes replace it. `SHOT_OPENS_WITH_TIME` refuses a time at the start of a shot's first beat, which would render as the removed header. `PROSE_TIME_NOT_INCREASING` and `PROSE_TIME_OUTSIDE_DURATION` apply base 4.2's two constraints to the times that remain. A time inside quoted on-screen text is excluded by position, for the reason `SHOT_HEADER_IN_PROSE` gives. The negatives run on a mid-shot split, a quoted timecode overlay, and a bare time beside an identical quoted one. The planner writes a mid-shot time only when the request asks for one.
...
- **Both system prompt preambles are claimed in the contract, and deliberately carry no anchor.** Owner ruling, 2026-09-06, closing F1. The text before the first heading of each prompt — the model's job, and for the planner the statement of invariant 1 — was described by no spec entry and reachable by no test, because `test/contract.test.ts` slices each block from its heading forward and a preamble has none. It is now `prompts.planner.preamble` and `prompts.patch.preamble`, with the reason for leaving it unasserted written into `noAnchor`.
  ...
  The ruling went against protecting it, and the measurements are why. A wording anchor is refuted by the same argument that gives `# How to write` its own `noAnchor`: a reworded preamble that says the same thing better turns it red, which is a change detector rather than a test. A presence floor survives that rewrite, but stays green through a preamble gutted of its meaning and refilled to length — so it is a tripwire against outright deletion, not protection of the invariant, and a green check next to the planner's most load-bearing paragraph would read as more than it is.

  The entry is read rather than decorative. `source`, the shot-header pattern and `open`/`close`/`join` were each declared and read by nothing in this same file, so an unread `preamble` key would have been the fourth instance and the worst, having been added specifically to look like protection. The reader asserts the reason exists; it deliberately does not assert the preamble does, and says so.

- **`beats-carry-prose` names both of its enforcement surfaces.** The invariant entry credited the absence of an enum-to-prose path in `src/core/serialize` and was silent on the instruction half, so a reader going to the entry written to explain invariant 1 would not find the preamble that states it. Now named as a pointer, and explicitly not as a control — deleting the preamble still turns nothing red.

- **The thinking default is `auto`, and the app can now express a thinking value at all.** Owner ruling, 2026-09-06. `auto` sends no `thinking` field, so heylook's own cascade decides — since 1.79.62 that resolves to the model's `models.toml` flag and then to whether it can think at all. `off` was what shipped and was never a verdict: it rested on one measurement on one 27B gguf, and it was an override rather than a default, because the client sent an explicit `false` to every model whose row advertises the switch.

  The wiring is what makes the ruling meaningful rather than nominal. `heylookPolicyConfig` carries `backpressureBudgetMs` alone and `ClientParams` had no thinking field, so until now every client constructed with the module default and nothing could change it — the ruling would have been unimplementable in either direction. `useEngine` now holds the preference as a stored setting, reads it defensively because a previous build could have written a boolean where the union is three-valued, hands it to `buildClient`, and the heylook panel offers the three states with `auto` explained as not-a-synonym-for-off.

  Two tests changed rather than being added, which is the honest record: both asserted the old default. `asks for thinking off only where the model has the switch` became `sends no thinking field by default, on any model`, and it now asserts across all three model rows plus an explicit `off`, because the capability gate and the mode gate both suppress the field and only a row that HAS the switch tells them apart.
...
- **`PLAN.md`.** A tracked working to-do for the planner-prompt and local-model work: the decisions that are the owner's, the order to take the open questions in, what is deliberately not being done, and what was relayed to the sister project on the first day of sharing between the two. Tracked rather than kept under `internal/` so the sister project can read it.
...
- **Thinking has three states, and the new one sends nothing.** `ThinkingPreference` was a boolean plus a depth, so the only way to say "do not ask this model to think" and the only way to say "I have no opinion" were the same value -- and the client sent that value to every model whose row advertises the switch. Since heylook 1.79.62 an absent `thinking` field resolves to the model's own `models.toml` flag and then to whether it can think at all, so what looked like a default was an override: h3-te was reaching across and turning thinking off on capable models that the server had decided should think.

  `ThinkingMode` is `auto | on | off`. `auto` omits the field entirely, including the depth -- an effort without a switch half-overrides the thing `auto` exists to leave alone. `off` still means false and is still the shipped default, because turning thinking on across the board changes generation cost and output and that is an owner call rather than a side effect of naming the third state. `scripts/conformance-heylook.mjs` takes `--thinking=auto` as a third arm, so the comparison can be run rather than argued.

  The breakage was run and it discriminates: dropping the `mode !== 'auto'` guard so the switch is always sent turns exactly one assertion red, the new one, and leaves the other 61 in that file green. The test asserts against the model row that HAS the capability, since on a row without it every mode omits the field and the check would pass while proving nothing.

- **Thinking is a per-client preference on heylook, not a constant off.** `ThinkingPreference` on `HeylookClientConfig` carries whether to ask a thinking model to think and, where the row advertises `reasoning_effort`, how hard. The app still sends off, which is what shipped, but off is no longer a verdict: the one measurement behind it was a single 27B gguf, and the owner's view is that thinking may account for a good part of the prose quality. The conformance harness takes `--thinking=on|off` and `--effort=<value>` so the comparison can be run rather than argued. Depth is sent only with thinking on and only where the capability is advertised, since a value the chat template does not know returns a 500. Tested at the builder and, separately, from construction to the wire body through a stubbed transport, because a preference held on the client and never handed to the builder would leave every builder test green and every real call thinking-off; dropping the hand-off turns that test red.

- **`null` for camera amplitude and speed means absent.** The prompt says medium and normal are expressed by leaving the field out, and a model doing exactly that in JSON writes `null`. Measured on 2026-09-01 with the harness: three of eight T2VA plans from a local model were refused on `amplitude: null` and nothing else, and the same field is what a 0.8B failed on in the 2026-08-31 note. The planner schema accepts the null, `assemble` drops it, so the document never carries one and the vocabulary stays the guide's two values. The JSON schema the trailer sends now advertises the null, which is what tells the model it is legal; a test reads that out of the generated schema rather than out of zod.

- **The conformance harness stores the document and takes two more switches.** Each row now carries the assembled document beside the rendered text, so a diagnostic can be read against the field that caused it. `--only=key,key` selects ideas, `--strip-example` removes the worked-example block before sending as an A/B on that block, and the thinking switches above. `CLAUDE.md` records that heylook serves one generation at a time, so one instrument runs at a time and never beside a generating browser tab: a second caller queues, and past the backpressure budget lands in the `provider` column looking like a model failure.

- **The planner is told the speaker id goes on every beat that speaks.** `SPEAKER_REF_MISSING_IN_PROSE` checks each dialogue-carrying beat for its `(Sn)`, and the prompt said only "write the id in the prose too", which a model reads as once. Measured with the conformance harness on 2026-09-01: a local model wrote `(S1)` and `(S2)` on first appearance and nothing on the three later lines of a two-speaker scene, and the document was refused for it. A prompt that cannot satisfy the validator it feeds is a defect whatever the id rule's standing; base 4.4 states that a speaker keeps the same id across shots and the ref guide's worked example writes it on every speaking sentence. Bound in the spec's `# Speech` note and by a marked wording proxy in the contract test, and the breakage was run.
...
- **`DIALOGUE_BAD_TERMINAL` is narrowed to the sentence it cites.** The rule demanded a terminal mark on every line it examined, so a planner-written chant or interjection failed. ref 5.4 asks for one only on "complete statements, questions, and exclamations", and the guide prints its own counterexample at `ref-en:272` -- a reused-audio lyric, `<d>[English] I'm lonely lonely lonely lonely lonely I'm lonely</d>`, with no terminal mark. A document the guide itself prints is not provably malformed, which is the bar a diagnostic has to clear here.
  ...
  The field is `fragment` and not `sung`, which was the first proposal and was a proxy. Completeness is the property ref 5.4 names and delivery only correlates with it: a sung line can be a complete statement and should be punctuated, a spoken interjection is a fragment and should not be, so a delivery flag leaks in both directions. Both leaks are now asserted. `fragment` joins `crossesCut === 'starts'` and `cutoff` in one incompleteness disjunction rather than arriving as a fourth unrelated flag, so the next case -- chanted, interrupted, muttered off -- resolves to the same property instead of needing a fifth.
...
- **Two spec notes, both recording a distinction an outside audit got wrong from the outside.** No behaviour changes.
  ...
  `# Camera` now says that `CAMERA_TYPES` is an annotation vocabulary and not a filter on prose. The closed set is compared only to `shots[].camera.type` in `cameraTypeValid`, which returns early on a null annotation, and `CAMERA_TYPES` is imported by that one rule and nothing else in `validate/` or `serialize/`. base 4.3's table is a set of motion concepts, and base-en:117 immediately says to render them "as a natural English action within the shot, rather than stacked as separate labels" -- which the prose does, unconstrained. So a motion the table cannot name, such as the focus pull in MiniMax's own fl2va reproducible payload, is written freely with no annotation and validates clean. Checked rather than argued: all five vendor worked examples in `test/fixtures` produce zero diagnostics. The general form of the outside finding does hold and is recorded with it -- closed-set enforcement cannot be pointed at ingested prose, so taking vendor payloads or an outside corpus as fixtures would mean extracting the annotation, not matching the text.

  `notWrittenByHand` now says that hand-written is not the same as unchecked. The concern it drew was that a long hand-maintained vocabulary rots and then fails for the wrong reason, which is a fair thing to fear and is the failure `binds` already removed: `vocabulary.cameraTypes.values` is compared to the `CAMERA_TYPES` export rather than trusted as a transcription. The hand copy is what lets the spec contradict the code; the binding is what stops it drifting silently. Neither alone would do, and the blanket sentence read as claiming only the first.
...
- **A sung line now has a path.** The `# Speech` block forbade naming a vocal act without supplying its words -- `sings` is in that list -- and then never said how sung words are supplied, so the only mention of singing anywhere in the prompt was a prohibition. A request carrying lyrics had nowhere to put them, and `soundscape` separately tells the model not to repeat singing there. ref 5.1 STATES the answer, "Write dialogue and lyrics as `<d>[Language] ...</d>`", and base 4.4 already treats lyrics as dialogue where one crosses a cut, so a sung line is a dialogue line: lyrics go in the beat's `dialogue` field, the singer takes a speaker id like any other voice, and two singing together take `(S1,S2)`. The block's citation is now `base 4.4, ref 5.1`, and the mixed cite was checked rather than assumed -- pointing the second part at a section that does not exist turns the citation walker red.
  ...
  The prompt says neither guide names a separate tag for lyrics, without naming the tokens, and the test asserts no denylist against them. The claim is scoped to the guides deliberately: "there is no separate tag" is false about the release, which declares a lyrics pair, while "neither guide names one" is true and is what a writer needs. The pair is emitted on purpose in marker experiments elsewhere, so only the scoped form is correct under both readings. The release declares a lyrics token pair that neither guide mentions, and a denylist would have to write those strings into the repo to test for their absence -- naming a string is how a model learns it exists. Breakage run: deleting the rule turns its assertion red.
...
- **The debug bus's replacement for an oversized payload is itself bounded.** The replacement carried every key of the dropped payload at full length, so a payload large through its keys produced a replacement over the very cap it stood in for, and the comment claiming the cap was always satisfiable was false. The key list is now bounded in count and length, and the replacement is weighed again with the list dropped if it is still over. The first test written for it stayed green with the bound removed, because the fallback satisfied the byte assertion on its own; it now also requires that some keys survive, and removing the bound turns it red.

- **Two negative prose assertions that could not fail.** The 2026-08-31 test audit made a planner mode note unconditional and reworded it, and `adds no mode note when the record has no marks` stayed green: a `not.toContain` on prose passes harder the more the prose changes. A version reading every note off `GLITCH_MODE_NOTES` was tried and stayed green against the same mutation, since a reworded note matches no table entry either. Both that test and its patch-prompt twin are now composition checks -- the styled prompt must equal the bare prompt plus exactly the derived directive, and the marked patch prompt must end on the derived glitch directive -- and the mutation turns each red. The five positive wording assertions the audit listed are marked as wording proxies, which CLAUDE.md already required of them.
