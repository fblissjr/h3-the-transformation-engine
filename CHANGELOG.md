...
- **The module exports only what something calls.** `wildPresets` and `getPreset` were defined, exported and — in the first case — tested, with no caller anywhere; `getAnchor` and `getVisualPack` were superseded by the single `getVisual` lookup the moment packs and anchors shared an id space; `activeAxes` is used only by the rule next to it. All five are gone. This is the same shape as the dead schema fixed below, found by looking for it after.
