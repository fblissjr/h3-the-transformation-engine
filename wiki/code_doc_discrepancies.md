...
| **D-21** | Version ID Collision on Page Reload | Postmortem 3:94–106: module counter reset on reload | `src/db/versions.ts:52–98`<br>(highestSuffix query, since removed) | `nextId = 1` counter clobbered `workspace:v0001` on every browser reload | **Resolved in Code**: Atomic transaction reads max existing key |
...
| **D-27** | Versionless IndexedDB Opening & Self-Healing | `CLAUDE.md:49` | `src/crypto/secureStore.ts:114–121` (`vault`) | Specifying DB version 1 prevents `upgrade` if empty DB pre-exists on origin | **Superseded for documents**: the store moved to SQLite, so openHealed no longer exists; the rule still binds the key vault, which is the only IndexedDB left |
...
- **Code Invariant (superseded):** the browser queried the highest existing numeric suffix from the IndexedDB versions store inside the write transaction. Allocation now happens on the server inside one SQLite transaction with a single writer, so the race is prevented rather than avoided, and neither that function nor the store it read still exists. The trap is kept because the *shape* recurs: a timestamp is not an ordering either, and that was assumed unique twice in this codebase.
...
- **Code Invariant (partly superseded):** `src/crypto/secureStore.ts` (lines 114–121, `vault`) still never calls `openDB(name, 1)` — it opens versionless, inspects store and index existence, and bumps the version dynamically. The document half is gone: that store is SQLite now, where the equivalent guard is a stamped `PRAGMA user_version` and a read-only open on a mismatch rather than a repair.
