...
## What the spec's own claims are checked against

Every claim in `contract.json` answers two questions, and both are enforced. It says where it came from -- a `guide` citation or `house: true` -- and it says what would have to change to make it false, in `binds`. Provenance was checked long before binding was: a mutation sweep of 63 fields found 33 could be changed with the whole suite green, including nine vocabulary entries that named a real exported constant and were compared to nothing.

`binds` is keyed by claim key, so an entry making two claims binds both. There are four kinds. `export` compares the value to the constant the code uses. `render` compares it to the serialized golden output and names the modes it must appear in, because an unscoped search passes for almost any short string. `quotedIn` compares it to a pinned guide, which is the right oracle for a form the fixtures happen not to exercise -- `(S1,S2)` appears in no fixture and twice in the base guide. `unbound` records, in a sentence, why a claim is deliberately unchecked; that list is pinned, so adding to it is a decision someone writes down.

Prompt blocks additionally carry `asserts`: structural anchors -- a field name, a tag literal, a rendered shape -- checked against the slice of the built prompt between that heading and the next. Anchors are never sentences, and each must land in exactly one block, which is what keeps an anchor from passing while pointing at neighbouring text. A block with no reachable structural anchor says so in `noAnchor` instead of getting a wording assertion by default.

`binds` points at where to compare. It never carries the value, because a spec generated from the code agrees with the code by construction and is worth nothing.
