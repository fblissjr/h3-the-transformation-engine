/**
 * The guides' own worked examples, as finished prompt text.
 *
 * MiniMax teaches this format almost entirely by worked example: four Cases in
 * the base guide, one complete example in the ref guide, one per mode. These
 * are those five, reproduced byte for byte.
 *
 * They live in `src/` rather than in `test/` because the planner prompt shows
 * one of them to the model -- the prompt had 2,500 words of rules and no
 * instance of the artifact those rules describe. Everything that reads them
 * reads this file: the mode blocks in `provider/prompts/planner.ts`, the
 * serializer round-trip in `test/serialize.test.ts`, and the byte-exactness
 * check in `test/guide-fidelity.test.ts`, which compares them to the guide
 * files on disk. One copy, three readers, and the strongest of the three
 * bindings is the one against the vendor's own text.
 *
 * Do not edit these to match the serializer. They are the vendor's output and
