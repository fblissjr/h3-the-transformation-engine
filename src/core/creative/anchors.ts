 ...
 *
 * Anchors share the pack shape and the `R` prefix keeps them in the same id
 * space as the packs, so a visual selection is one string and one lookup rather
 * than a string-or-number union with a branch at every use.
 *
 * Source: the H3 Prompt Director Shareable Pack, style reference anchors.
 */

import type { PackDef } from './packs';

export const STYLE_ANCHORS = [
  // Animation and illustration anchors (R01-R20)
  {
    id: 'R01',
  ...
  // Live-action, commercial, and game anchors (R21-R30)
  {
    id: 'R21',
