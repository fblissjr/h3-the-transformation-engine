...
A sung line is a dialogue line. Put the lyrics in the beat's \`dialogue\` field exactly as you would spoken words, give the singer a speaker id like any other voice, and write (S1,S2) for two singing together. Neither guide names a separate tag for lyrics, so do not reach for one.
...
\`music\` covers score only the audience can hear. Decide for this scene whether it has one, and use "N/A" only when it does not. That is a judgement about this video, not a habit: reaching for "N/A" by reflex asserts that every clip you write is unscored, and writing a score nobody asked for makes a claim about the scene that was never made. When there is one, describe it in ${MUSIC_SENTENCE_RANGE[0]}-${MUSIC_SENTENCE_RANGE[1]} sentences: name instrumentation, tempo, rhythm and dynamics, never mood words like "emotional" or "epic". Music a character can hear is a diegetic event and belongs in the beats, not here.`;
